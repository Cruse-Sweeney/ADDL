...
def block_dequantize_2bit(x_quant_2: torch.Tensor, normalization: torch.Tensor) -> torch.Tensor:
    ...
    unpacked = torch.zeros(x_quant_2.size(0), x_quant_2.size(1) * 4, dtype=torch.float32)
    for i in range(4):
...
class Linear2Bit(torch.nn.Module):
    ...
    @staticmethod
    def _load_state_dict_pre_hook(module, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        if f"{prefix}weight" in state_dict:
            weight = state_dict[f"{prefix}weight"]
            del state_dict[f"{prefix}weight"]

            flat_weight = weight.contiguous().view(-1)
            assert flat_weight.numel() % module._group_size == 0
